						...
						<?php for($j = 1 ; $j <= $miejsca ; $j++): ?>
							...
							<?php foreach($seats as $seat)
							{
								if($seat['Seat']['num_rown'] == $i && $seat['Seat']['num_seat'] == $j)
								{
									...
									{
										if(in_array('5269',$ownerseatId))
										{
											CakeLog::write('debug', 'myArray22222'.print_r( $ownerseatId, true) );
										}
										foreach($rezs as $rez)
										{
											if($seat['Seat']['id'] == $rez['SeatsReservation']['Seats_id'] && !in_array($seat['Seat']['id'],$seatid))
											{
												?>
											...
											else if ($seat['Seat']['id'] == $rez['SeatsReservation']['Seats_id'] && in_array($seat['Seat']['id'],$seatid))
											{
												?>
												<div class="miejsce_wybrane" id="<?php echo $seat['Seat']['id']?>">
													<span class="wartosc"><?php echo "$numer" ;$numer++;$wejsce = true;?></span>
												</div>
												<?php
											}
										}
...
<!-- Bootstrap Core JavaScript -->
	...
	<script>
		...
		var tab =[];
		var idx = 0;
		var wybrane = ilosc['Reservation']['count_seats_reserv'];
		var cena_og = ilosc['Reservation']['price'];
		...
		window.onload = function()
		{
			...
			{
				var id = ownerseat[i]['SeatsReservation']['Seats_id'];
				var rzad = ownerseat[i]['SeatsReservation']['x'];
				var miejsce = ownerseat[i]['SeatsReservation']['y'];
				var typ = ownerseat[i]['SeatsReservation']['type'];

				tab.push({id: id, x: parseInt(rzad), y: parseInt(miejsce), type: typ}); // wpis do tablicy po click
				...
				zmienna = document.getElementById(ownerseat[i]['SeatsReservation']['Seats_id']);
						...
						ticketarea.setAttribute('id','Miejsce'+ownerseat[i]['SeatsReservation']['Seats_id']);
		...
		$('.miejsce,.miejsce_wybrane').click(function()
		{
			...
			if($(this).hasClass('miejsce'))
			{
				...
				var ni = document.getElementById('bilet');
				var ticketarea = document.createElement('div');
				ticketarea.setAttribute('id','Miejsce'+id);
				...
				if(check == false) {
					tab.push({id: id, x: parseInt(rzad), y: parseInt(miejsce), type: "Norm"}); // wpis do tablicy po click
			...
			else
			{
				...
				var olddiv = document.getElementById('Miejsce'+id);
			...
			if(tab.length == 0)
			{
				...
				$.ajax({
					...
					data: {"Seat": tab, Screen_id:<?php echo $screen['Screen']['id']?>,Movie_id:<?php echo $screen['Screen']['Movies_id']?>,price:cena_og,
						count:wybrane,Reservation_id: <?php echo $Reservation_id?> },
					url: "/CakeCinema/Reservations/editcontent/",
					success: function () {
